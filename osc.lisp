...
;; EDITED subseconds given as fractions of a second now.
...
(defun encode-timetag (timetag)
  ...
  (cond
    ...
    (t
     (let ((split (multiple-value-list (floor timetag))))
       (cat (encode-int32 (first split))
	    (encode-int32 (round (* (second split) (expt 256 4)))))))))
